...
mod mysql {
    ...
    fn executable_comment_syntax() {
        let conn = prepare();
        let sqls = vec![
            //(query!("SELECT 1 ") + "/*! +1 */", "SELECT 1 '/*! +1 */'", "1"), <- syntax error
            (query!("SELECT 1 /*! +1 */"), "SELECT 1 /*! +1 */", "2"),
            (query!("SELECT /*! 42 */"), "SELECT /*! 42 */", "42"),
            (
                query!("SELECT ") + "/*! 42 */",
                "SELECT '/*! 42 */'",
                "/*! 42 */",
    ...
    fn blob() {
        let conn = concatsql::mysql::open("mysql://localhost:3306/test").unwrap();
        conn.execute("CREATE TEMPORARY TABLE b (data blob)")
            .unwrap();
    ...
    fn sql_injection() {
        ...
        assert_eq!(
            conn.rows(&sql),
            Err(Error::Message(
                "exec error: DriverError { Statement takes 0 parameters but 1 was supplied }"
                    .to_string()
            ))
        );
        ...
        let sql = query!("SELECT ") + "CONCAT(login, password)";
        for row in conn.rows(&sql).unwrap() {
            assert_eq!(row.get(0).unwrap(), "CONCAT(login, password)");
        }

        let sql = query!("SELECT ") + "CONCAT('0x',HEX('c:\\\\boot.ini'))";
        for row in conn.rows(&sql).unwrap() {
            assert_eq!(row.get(0).unwrap(), "CONCAT('0x',HEX('c:\\\\boot.ini'))");
        }

        let sql = query!("SELECT ") + "CONCAT(CHAR(75),CHAR(76),CHAR(77))";
        for row in conn.rows(&sql).unwrap() {
            assert_eq!(row.get(0).unwrap(), "CONCAT(CHAR(75),CHAR(76),CHAR(77))");
        }

        let sql = query!("SELECT ") + "LOAD_FILE(0x633A5C626F6F742E696E69)";
        for row in conn.rows(&sql).unwrap() {
