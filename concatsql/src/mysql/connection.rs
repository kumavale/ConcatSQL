...
impl ConcatsqlConn for RefCell<mysql::Conn> {
    fn execute_inner<'a>(
        ...
        if params.is_empty() {
            ...
            let params = params
                .iter()
                .map(|value| to_mysql_value!(value))
                .collect::<Vec<_>>();
            ...
            ($result:expr) => {
                while let Some(result_set) = $result.next_set() {
                    ...
                    let mut pairs: Vec<(String, Option<String>)> =
                        Vec::with_capacity(result_set.affected_rows() as usize);
            ...
            ($result:expr, $rows:expr) => {
                if let Some(result_set) = $result.next_set() {
                    let result_set = match result_set {
                        Ok(result_set) => result_set,
                        Err(e) => {
                            return Error::new(error_level, "exec error", &e).map(|_| Vec::new())
                        }
                    };
                    ...
                    for result_row in result_set {
                        let result_row = match result_row {
                            Ok(row) => row,
                            Err(e) => {
                                return Error::new(error_level, "exec error", &e)
                                    .map(|_| Vec::new())
                            }
                        };
...
impl GetToString for mysql::Row {
    fn get_to_string(&self, index: usize) -> Option<String> {
        match self[index] {
            ...
            mysql::Value::Time(neg, days, hours, minutes, seconds, micros) => Some(if neg {
                format!(
                    "-{:03}:{:02}:{:02}.{:06}",
...
mod tests {
    ...
    fn open() {
        ...
        assert_eq!(
            crate::mysql::open(""),
            Err(Error::Message(
                "failed to open: URL ParseError { relative URL without a base }".into()
            ))
        );
        assert_eq!(
    ...
    fn get_to_string() {
        ...
        ").unwrap();
        assert_eq!(
            conn.rows("SELECT bytes FROM test")
                .unwrap()
                .first()
                .unwrap()
                .get(0)
        ...
        assert_eq!(
            conn.rows("SELECT   i32 FROM test")
                .unwrap()
                .first()
                .unwrap()
                .get(0)
        ...
        assert_eq!(
            conn.rows("SELECT   f32 FROM test")
                .unwrap()
                .first()
                .unwrap()
                .get(0)
        ...
        assert_eq!(
            conn.rows("SELECT   f64 FROM test")
                .unwrap()
                .first()
                .unwrap()
                .get(0)
        ...
        assert_eq!(
            conn.rows("SELECT  date FROM test")
                .unwrap()
                .first()
                .unwrap()
                .get(0)
        ...
        assert_eq!(
            conn.rows("SELECT  time FROM test")
                .unwrap()
                .first()
                .unwrap()
                .get(0)
